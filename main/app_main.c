...
#define EEPROM_SIZE 512 // Tamanho da EEPROM em bytes
#define SENHA_ADDR 0 // Endereço na EEPROM onde a senha será armazenada
...
void setup() 
{
  ...
  EEPROM.get(SENHA_ADDR, senha); // Lê a senha da EEPROM
  if (senha[0] == 0xFF) // Verifica se a EEPROM está vazia
  { 
    strcpy(senha, "1510"); // Define a senha padrão se a EEPROM estiver vazia
  }
}
    ...
    while(1)
    {
        ...
        if(ctrl == 1)
        {
            ...
            if(qdig == 4) // quando qdig == 4, deste modo todas as casas foram preenchidas é analisado se a senha está correta
            {
                ...
                if(adm2 == 1) // caso a senha esteja correta será executado o código abaixo
                {
                    ...
                    if(adm3 == 1 && qdig == 4)
                    {
                        EEPROM.put(SENHA_ADDR, senha); // salva a nova senha na EEPROM
                        EEPROM.commit();
        ...
        if(qdig == 4 && ctrl == 0) // quando qdig == 4, deste modo todas as casas foram preenchidas é analisado se a senha está correta
        {
            if(n1 == senha) // caso a senha não tenha sido trocada no painel ADM, no topo do código, ela terá seu valor padrão "1510"
            {
                ...
                {   
                    lcd595_clear();
                    lcd595_write(2,0, "COFRE ABERTO");

                    vTaskDelay(15000 / portTICK_PERIOD_MS); // tempo de 15 segundos antes que o cofre comece a fechar efetivamente
